# page-256kib-rgsize-10mib-file-10mib
...
This folder contains one subdirectory per row count, for example
`10000_rows/` and `1000000_rows/`. Each row-count directory contains
`results/` for overall markdown, `results/configs/` for per-configuration
markdown, `tsvs/` for tabular outputs, and `parquet/` as the temporary parquet
output root for that row count.
...
(`none`, `snappy`, `zstd`) times integer/date/timestamp encodings
(`plain`, `rle-dict`) times string encodings (`plain`, `rle-dict`,
`delta-byte-array`, `delta-length-byte-array`). It also writes aggregate TSV
files ranking experiments and selecting per-column winners.
